...
class LLaVA_Next(BaseModel):
    ...
    def _build_mmmu_prompt(self, line, dataset=None):
        assert self.use_custom_prompt(dataset)
        assert dataset is None or isinstance(dataset, str)
        tgt_path = self.dump_image(line, dataset)
    ...
    def _build_yorn_prompt(self, line, dataset: str) -> list[dict[str, str]]:
        ...
        if 'Please answer yes or no' not in question:
            ...
            question = question.replace('Yes or No', '\nChoices:\nA: Yes\nB: No')
        elif 'yes or no' in question:
            question = question.replace('yes or no', '\nChoices:\nA: Yes\nB: No')
        else:
            question = question
    ...
    def _process_yorn_prompt(self, answer):

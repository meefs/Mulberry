...
class llama_vision(BaseModel):
    ...
    def use_custom_prompt(self, dataset):
        if dataset is None:
            return False
        if listinstr(['AI2D', 'MMMU', 'MathVista', 'ChartQA', 'DocVQA', 'MMStar', 'HallusionBench'], dataset):
    ...
    def build_prompt(self, line, dataset=None):
        ...
        if listinstr(['AI2D'], dataset):
            ...
            if 'Please answer yes or no' not in question:
                prompt = f'{question}\nChoices:\nA: Yes\nB: No'
            elif 'Please answer yes or no.' in question:
                prompt = question.replace('Please answer yes or no.', '\nChoices:\nA: Yes\nB: No')
            elif 'Please answer Yes or No.' in question:
                prompt = question.replace('Please answer Yes or No.', '\nChoices:\nA: Yes\nB: No')
            elif 'Yes or No' in question:
                prompt = question.replace('Yes or No', '\nChoices:\nA: Yes\nB: No')
            elif 'yes or no' in question:
                prompt = question.replace('yes or no', '\nChoices:\nA: Yes\nB: No')
            else:
                prompt = question
    ...
    def generate_inner(self, message, dataset=None):
        ...
        prompt = REASONING_PROMPT.format(question=prompt)
        # print([prompt])
        ...
        output = self.processor.decode(output[0][inputs['input_ids'].shape[1]:]).replace('<|eot_id|>', '')
        if len(output.split('The final answer is:')) == 2 and dataset != 'MMVet':
            print(output)
            output = output.split('The final answer is:')[-1].strip()
